...
class UserTokensView(BaseAqueductView, TemplateView):
    ...
    def get_context_data(self, **kwargs) -> dict[str, object]:
        ...
        # token operates under ``user_limits`` independently; the rings report
        # that token's own consumed request budget.
        for token in tokens:
            token.usage = get_aggregate_usage([token.id])  # type: ignore[attr-defined]
        ...
        for sa in service_accounts:
            ...
            sa.usage = get_aggregate_usage([sa.token.id])  # type: ignore[attr-defined]
